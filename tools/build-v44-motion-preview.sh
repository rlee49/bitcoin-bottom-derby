...
out_dir="$project_dir/qa-v44/motion-preview-frames"
mkdir -p "$out_dir"
...
ffmpeg -y -hide_banner -loglevel error -framerate 10 -i "$out_dir/frame-%03d.png" \
  -c:v libx264 -pix_fmt yuv420p -movflags +faststart "$project_dir/qa-v44/running-track-preview.mp4"

ffmpeg -y -hide_banner -loglevel error -i "$project_dir/qa-v44/running-track-preview.mp4" \
  -vf "select='not(mod(n,6))',scale=520:455,tile=1x8:padding=4:margin=2:color=0x101820" \
  -frames:v 1 -pix_fmt rgb24 "$project_dir/qa-v44/runtime-storyboard.png"
